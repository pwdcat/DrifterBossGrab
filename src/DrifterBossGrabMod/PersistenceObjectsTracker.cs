    ...
    public static class PersistenceObjectsTracker
    {
        ...
        public static void TrackBaggedObject(GameObject obj)
        {
            ...
            lock (_lock)
            {
                ...
                {
                    Log.Info($"{Constants.LogPrefix} Tracking bagged object: {obj.name} (total tracked: {_currentlyBaggedObjects.Count})");
                }
            }
        }
        ...
        public static void ClearTrackedObjects()
        {
            lock (_lock)
            {
                ...
                if (PluginConfig.EnableDebugLogs.Value)
                {
                    Log.Info($"{Constants.LogPrefix} Cleared all tracked bagged objects");
                }
            }
        }
        ...
        public static void CleanupNullReferences()
        {
            lock (_lock)
            {
                ...
                {
                    Log.Info($"{Constants.LogPrefix} Cleaned up {removed} null references from bagged objects tracker");
                }
            }
        }
