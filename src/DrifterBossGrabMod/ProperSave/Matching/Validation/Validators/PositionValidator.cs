...
{
    public class PositionValidator : IObjectValidator
    {
        public int Priority => 70;

        public ValidationResult Validate(GameObject obj, BaggedObjectSaveData saveData)
        {
            // Return NoMatch if PrefabName is null or empty
            if (string.IsNullOrEmpty(saveData.PrefabName))
            {
                return NoMatch();
            }
            ...
            // Return NoMatch if saved position is zero
            if (savedPosition == Vector3.zero)
            {
                return NoMatch();
            }

            // Return NoMatch if object name doesn't match exactly
            if (!string.Equals(obj.name, saveData.PrefabName))
            {
                return NoMatch();
            }

            // Calculate distance between saved and current position
            float distance = Vector3.Distance(obj.transform.position, savedPosition);

            // Check if within tolerance
            if (distance <= ObjectValidator.POSITION_TOLERANCE)
            {
                // Calculate confidence: 1.0 at 0m, 0.7 at tolerance
                float confidence = 1.0f - (distance / ObjectValidator.POSITION_TOLERANCE) * ProperSaveConstants.Validation.PositionMatchWeight;

                return new ValidationResult
                {
                    Confidence = confidence,
                    Method = MatchMethod.PrefabNameAndPosition,
                    Reason = $"Prefab name match at {distance:F2}m"
                };
            }
