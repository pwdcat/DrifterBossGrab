...
namespace DrifterBossGrabMod
{
    public class BottomlessBagFeature : FeatureToggleBase
    {
        ...
        protected override void ApplyPatches(Harmony harmony)
        {
            ...
            harmony.CreateClassProcessor(typeof(Patches.BaggedObjectPatches.BaggedObject_OnEnter)).Patch();

            harmony.CreateClassProcessor(typeof(Patches.BaggedObjectPatches.BaggedObject_OnExit)).Patch();
            harmony.CreateClassProcessor(typeof(Patches.BaggedObjectPatches.BaggedObject_FixedUpdate)).Patch();
            harmony.CreateClassProcessor(typeof(Patches.BaggedObjectPatches.VehicleSeat_FixedUpdate)).Patch();
            harmony.CreateClassProcessor(typeof(Patches.BaggedObjectPatches.VehicleSeat_EjectPassenger)).Patch();
            
            // Newly identified missing patches
            harmony.CreateClassProcessor(typeof(Patches.BaggedObjectPatches.EntityStateMachine_SetNextStateToMain)).Patch();
            harmony.CreateClassProcessor(typeof(Patches.BaggedObjectPatches.EntityState_OnEnter_Patch)).Patch();
            harmony.CreateClassProcessor(typeof(Patches.BaggedObjectPatches.GenericSkill_ExecuteIfReady_Patch)).Patch();

            // Bag transition/cleanup patches (Note: these are NOT nested in BagPatches)
